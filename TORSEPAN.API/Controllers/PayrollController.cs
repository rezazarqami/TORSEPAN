...
public sealed class PayrollController(TORSEPANDbContext db, IHttpClientFactory httpFactory, IConfiguration configuration) : ControllerBase
{
    ...
    public async Task<IActionResult> SaveRate(PayrollRateRequest request, CancellationToken ct)
    {
        ...
        if (request.Id.HasValue)
        {
            ...
            rate.Update(action, materialId, bowlType, scaleId, request.IsExport, request.IsCustom, request.Amount);
        }
        var matches = await db.PayrollRates.Where(x => x.Action == action && x.MaterialId == materialId &&
            x.BowlType == bowlType && x.ScaleId == scaleId && x.IsExport == request.IsExport && x.IsCustom == request.IsCustom &&
            (!request.Id.HasValue || x.Id != request.Id.Value)).ToListAsync(ct);
        ...
        {
            db.PayrollRates.Add(new PayrollRate(action, materialId, bowlType, scaleId, request.IsExport, request.IsCustom, request.Amount));
        }
        ...
        bool readyForExportPackaging, bool exportWarehouse, CancellationToken ct)
    {
        ...
        }).Select(g =>
        {
            var rate = g.Key.Action == ProductionAction.Design && g.Key.DesignTypeId.HasValue && designTypes.TryGetValue(g.Key.DesignTypeId.Value, out var designType)
                ...
                : rates.Where(r => r.IsExport == g.Key.IsExport && r.IsCustom == g.Key.IsCustom && r.Action == g.Key.Action && (!r.MaterialId.HasValue || r.MaterialId == g.Key.MaterialId) && (!r.BowlType.HasValue || (int)r.BowlType == g.Key.BowlType) && (!r.ScaleId.HasValue || r.ScaleId == g.Key.ScaleId))
            ...
            var count = g.Key.Action == ProductionAction.Glue ? g.Where(x => x.HandpanId.HasValue).Select(x => x.HandpanId).Distinct().Count()
                : g.Count();
            return new PayrollLine(g.Key.UserId, string.IsNullOrWhiteSpace(g.Key.FullName) ? g.Key.UserName : g.Key.FullName, g.Key.DisplayOrder, (int)g.Key.Action, Title(g.Key.Action), g.Key.MaterialId, g.Key.Material, g.Key.BowlType, g.Key.ScaleId, g.Key.Scale, count, rate, count * rate, g.Key.IsExport, g.Key.IsCustom);
        ...
        return new PayrollCalculation(start, end.AddDays(-1), lines, users,
            rates.Select(r => new PayrollRateDto(r.Id, (int)r.Action, Title(r.Action), r.MaterialId, r.Material?.Name ?? "همه متریال‌ها", r.BowlType.HasValue ? (int)r.BowlType : null, r.ScaleId, r.Scale?.Name ?? "", r.IsExport, r.IsCustom, r.Amount)).ToList(),
...
public sealed record PayrollLine(Guid UserId, string UserName, int DisplayOrder, int Action, string ActionTitle, Guid? MaterialId, string MaterialName, int? BowlType, Guid? ScaleId, string ScaleName, int Count, decimal Rate, decimal Total, bool IsExport = false, bool IsCustom = false);
public sealed record PayrollUser(Guid Id, string FullName, int DisplayOrder);
public sealed record PayrollRateDto(Guid Id, int Action, string ActionTitle, Guid? MaterialId, string MaterialName, int? BowlType, Guid? ScaleId, string ScaleName, bool IsExport, bool IsCustom, decimal Amount);
public sealed record PayrollRateRequest(Guid? Id, int Action, Guid? MaterialId, int? BowlType, Guid? ScaleId, bool IsExport, bool IsCustom, decimal Amount);
