...
public sealed class ProductionService
{
    ...
    public async Task<SaleOperationResult?> SellManyAsync(IReadOnlyCollection<Guid> ids,string? buyerName,string? buyerPhoneNumber,decimal? price,string? destination,Guid? partyId,bool isPaid,DateTime? dueDate,bool activateWarranty=false) => await _api.PostAsync<object,SaleOperationResult>("production/sales/bulk",new { HandpanIds=ids,Sale=new { BuyerName=buyerName,BuyerPhoneNumber=buyerPhoneNumber,Price=price,Destination=destination,PartyId=partyId,IsPaid=isPaid,DueDate=dueDate,ActivateWarranty=activateWarranty } });
    public async Task UpdateSaleAsync(Guid id,string? buyerName,string? buyerPhoneNumber,decimal? price,string? destination,Guid? partyId,bool isPaid,DateTime? dueDate,bool activateWarranty=false) => await _api.PutAsync<object,object?>($"production/{id}/sale",new { BuyerName=buyerName,BuyerPhoneNumber=buyerPhoneNumber,Price=price,Destination=destination,PartyId=partyId,IsPaid=isPaid,DueDate=dueDate,ActivateWarranty=activateWarranty });
