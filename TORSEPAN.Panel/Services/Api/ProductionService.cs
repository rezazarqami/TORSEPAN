...
public sealed class ProductionService
{
    ...
    public async Task<SaleOperationResult?> SellAsync(Guid id,string? buyerName,decimal? price,string? destination,Guid? partyId,bool isPaid,DateTime? dueDate,bool activateWarranty=false,string? warrantyFullName=null,string? warrantyPhoneNumber=null,string? warrantyCity=null) => await _api.PostAsync<object,SaleOperationResult>($"production/{id}/sell",new { BuyerName=buyerName, Price=price, Destination=destination,PartyId=partyId,IsPaid=isPaid,DueDate=dueDate,ActivateWarranty=activateWarranty,WarrantyFullName=warrantyFullName,WarrantyPhoneNumber=warrantyPhoneNumber,WarrantyCity=warrantyCity });
    public async Task<SaleOperationResult?> SellManyAsync(IReadOnlyCollection<Guid> ids,string? buyerName,decimal? price,string? destination,Guid? partyId,bool isPaid,DateTime? dueDate,bool activateWarranty=false,string? warrantyFullName=null,string? warrantyPhoneNumber=null,string? warrantyCity=null) => await _api.PostAsync<object,SaleOperationResult>("production/sales/bulk",new { HandpanIds=ids,Sale=new { BuyerName=buyerName,Price=price,Destination=destination,PartyId=partyId,IsPaid=isPaid,DueDate=dueDate,ActivateWarranty=activateWarranty,WarrantyFullName=warrantyFullName,WarrantyPhoneNumber=warrantyPhoneNumber,WarrantyCity=warrantyCity } });
    public async Task UpdateSaleAsync(Guid id,string? buyerName,decimal? price,string? destination,Guid? partyId,bool isPaid,DateTime? dueDate,bool activateWarranty=false,string? warrantyFullName=null,string? warrantyPhoneNumber=null,string? warrantyCity=null) => await _api.PutAsync<object,object?>($"production/{id}/sale",new { BuyerName=buyerName,Price=price,Destination=destination,PartyId=partyId,IsPaid=isPaid,DueDate=dueDate,ActivateWarranty=activateWarranty,WarrantyFullName=warrantyFullName,WarrantyPhoneNumber=warrantyPhoneNumber,WarrantyCity=warrantyCity });
