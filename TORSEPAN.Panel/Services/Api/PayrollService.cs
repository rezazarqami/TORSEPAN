...
public sealed class PayrollService(ApiClient api)
{
 ...
 public async Task SaveRateAsync(int action,Guid? materialId,int? bowlType,decimal amount)=>await api.PostAsync<object,object?>("payroll/rates",new{Action=action,MaterialId=materialId,BowlType=bowlType,Amount=amount});
