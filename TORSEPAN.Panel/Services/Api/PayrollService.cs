...
public sealed class PayrollService(ApiClient api)
{
 ...
 public async Task SaveRateAsync(Guid? id,int action,Guid? materialId,int? bowlType,Guid? scaleId,decimal amount)=>await api.PostAsync<object,object?>("payroll/rates",new{Id=id,Action=action,MaterialId=materialId,BowlType=bowlType,ScaleId=scaleId,Amount=amount});
