...
{
 public Task<PayrollDto?> GetAsync(DateTime? from=null,DateTime? to=null,bool readyForQc=false,bool readyForPackaging=false,bool enteredWarehouse=false,bool readyForExportPackaging=false,bool exportWarehouse=false){var q=new List<string>();if(from.HasValue)q.Add($"from={from.Value.ToString("yyyy-MM-dd",CultureInfo.InvariantCulture)}");if(to.HasValue)q.Add($"to={to.Value.ToString("yyyy-MM-dd",CultureInfo.InvariantCulture)}");if(readyForQc)q.Add("readyForQc=true");if(readyForPackaging)q.Add("readyForPackaging=true");if(enteredWarehouse)q.Add("enteredWarehouse=true");if(readyForExportPackaging)q.Add("readyForExportPackaging=true");if(exportWarehouse)q.Add("exportWarehouse=true");return api.GetAsync<PayrollDto>("payroll"+(q.Count>0?"?"+string.Join("&",q):""));}
 public async Task SaveRateAsync(Guid? id,int action,Guid? materialId,int? bowlType,Guid? scaleId,bool isExport,decimal amount)=>await api.PostAsync<object,object?>("payroll/rates",new{Id=id,Action=action,MaterialId=materialId,BowlType=bowlType,ScaleId=scaleId,IsExport=isExport,Amount=amount});
