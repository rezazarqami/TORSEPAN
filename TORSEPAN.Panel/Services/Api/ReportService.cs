...
public sealed class ReportService
{
    ...
    public async Task<InventoryReportDto> GetInventoryAsync(DateTime? from,DateTime? to,string kind,IReadOnlyCollection<Guid> materials,IReadOnlyCollection<Guid> scales,string destination,string notes)
        => await _api.GetAsync<InventoryReportDto>("management-reports/inventory"+Query(from,to,materials,scales,"all",destination,$"kind={Uri.EscapeDataString(kind)}&notes={Uri.EscapeDataString(notes)}"))??new();

    public Task<byte[]> PdfAsync(string kind,DateTime? from,DateTime? to,IReadOnlyCollection<Guid> materials,IReadOnlyCollection<Guid> scales,string payroll,string destination,string inventoryKind,Guid? userId,int? action,string notes)
    {
        var extra=$"inventoryKind={Uri.EscapeDataString(inventoryKind)}&notes={Uri.EscapeDataString(notes)}"+(userId.HasValue?$"&userId={userId}":"")+(action.HasValue?$"&action={action}":"");
        return _api.GetBytesAsync($"management-reports/{kind}/pdf"+Query(from,to,materials,scales,payroll,destination,extra));
    }

    public Task SendTelegramAsync(string kind,DateTime? from,DateTime? to,IReadOnlyCollection<Guid> materials,IReadOnlyCollection<Guid> scales,string payroll,string destination,string inventoryKind,Guid? userId,int? action,string notes)
        => _api.PostAsync<object,object?>($"management-reports/{kind}/telegram",new{From=from,To=to,MaterialIds=materials,ScaleIds=scales,Payroll=payroll,Destination=destination,InventoryKind=inventoryKind,UserId=userId,Action=action,NotesFilter=notes});
