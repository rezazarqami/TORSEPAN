...
public sealed class AccountingDocument : Entity
{
    ...
    public void UpdateSale(decimal total,decimal paid,Guid? partyId,DateTime? dueDate,string? notes)
    {if(total<0||paid<0||paid>total)throw new ArgumentOutOfRangeException(nameof(total));TotalAmount=total;PaidAmount=paid;PartyId=partyId;DueDate=dueDate?.ToUniversalTime();Notes=string.IsNullOrWhiteSpace(notes)?null:notes.Trim();SettledAt=paid==total?DateTime.UtcNow:null;}
}
